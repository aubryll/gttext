...
LRESULT CMainWindow::OnFromScannerOrCamera(HWND parent)
{
    HRESULT hr;
    ...
    {
		if(!m_scannedImage.IsNull())
		{
			m_scannedImage.Detach();
		}
		m_scannedImage.Load(ppStream[i]);
...
BOOL CMainWindow::IsEnabled()
{
	if (this->m_pEventCallback != NULL)
	{
		LONG nNumDevices = this->m_pEventCallback->GetNumDevices();

		BOOL uEnable = nNumDevices > 0 ? TRUE : FALSE;
		return uEnable;
	}
	return FALSE;
}
